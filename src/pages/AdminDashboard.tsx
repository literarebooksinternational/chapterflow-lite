...
export default function AdminDashboard() {
  ...
  const updateEnvio = async (id: string, updates: Partial<EnvioCapitulo>) => {
    try {
      ...
      // OTIMIZAÇÃO: Atualização de estado local sem refetch.
      // Isso torna a UI mais rápida e evita uma chamada desnecessária à API.
      setEnvios(prev => prev.map(envio => {
        if (envio.id === id) {
          const responsibleProfile = updates.responsible_user_id
            ? profiles.find(p => p.user_id === updates.responsible_user_id)
            : null;
          
          return { 
            ...envio, 
            ...updates,
            responsible_profile: responsibleProfile || null
          };
        }
        return envio;
      }));
  ...
  // OTIMIZAÇÃO: Estado derivado. A lista filtrada é calculada diretamente na renderização.
  // Isso remove a necessidade de um useState e useEffect extras, simplificando o código.
  const filteredEnvios = envios.filter(envio => {
    const lowerCaseSearch = searchTerm.toLowerCase();
    
    if (statusFilter !== 'all' && envio.status !== statusFilter) {
      return false;
    }
    if (responsibleFilter !== 'all' && envio.responsible_user_id !== responsibleFilter) {
      return false;
    }
    if (searchTerm && !(
      envio.nome.toLowerCase().includes(lowerCaseSearch) ||
      envio.email.toLowerCase().includes(lowerCaseSearch) ||
      envio.livro.toLowerCase().includes(lowerCaseSearch) ||
      envio.titulo_capitulo.toLowerCase().includes(lowerCaseSearch)
    )) {
      return false;
    }
    return true;
  ...
  return (
    <div className="min-h-screen gradient-dark">
      {/* CORREÇÃO: Layout do cabeçalho ajustado para alinhar itens corretamente */}
      <header className="border-b border-glass-border" style={{ backgroundColor: 'hsl(0 0% 8%)' }}>
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            ...
            {/* Lado Direito: Avatar e Botão Sair */}
            <div className="flex items-center space-x-4">
      ...
      <div className="container mx-auto px-4 py-8">
        <div ref={dashboardRef} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              <CardContent className="p-4 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-2xl font-bold">{envios.filter(e => e.status === 'Recebido').length}</p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-2xl font-bold">{envios.filter(e => e.status === 'Em Análise').length}</p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-2xl font-bold">{envios.filter(e => e.status === 'Aprovado').length}</p>
          ...
          {/* Filters and Actions */}
          <Card>
            <CardHeader><CardTitle>Filtros e Ações</CardTitle></CardHeader>
            <CardContent>
              <div className="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={statusFilter} onValueChange={(value: StatusEnvio | 'all') => setStatusFilter(value)}>
                    <SelectTrigger className="w-full md:w-[180px] glass"><SelectValue placeholder="Filtrar por status" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={responsibleFilter} onValueChange={setResponsibleFilter}>
                    <SelectTrigger className="w-full md:w-[180px] glass"><SelectValue placeholder="Filtrar por responsável" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">Todos</SelectItem>
                      {profiles.map((profile) => (
                        <SelectItem key={profile.user_id} value={profile.user_id}>{profile.display_name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex space-x-2">
                  <Button onClick={fetchEnvios} variant="outline"><RefreshCw className="h-4 w-4 mr-2" />Atualizar</Button>
                  <Button onClick={exportToCSV} variant="outline"><FileDown className="h-4 w-4 mr-2" />Exportar CSV</Button>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Submissions Table */}
          <Card>
            <CardHeader><CardTitle>Envios de Capítulos ({filteredEnvios.length})</CardTitle></CardHeader>
            <CardContent>
              {isLoading ? (
                <div className="flex items-center justify-center py-8"><RefreshCw className="h-6 w-6 animate-spin text-editorial" /></div>
              ) : (
                <div className="overflow-x-auto">
                          ...
                          <TableCell>
                            {editingEnvio === envio.id ? (
                              <div className="space-y-2 min-w-[200px]">
                                <Select value={editData.status} onValueChange={(value: StatusEnvio) => setEditData(p => ({ ...p, status: value }))}>
                                ...
                                <Textarea
                                  ...
                                  onChange={(e) => setEditData(p => ({ ...p, observacao: e.target.value }))}
                                  rows={2}
                                />
                                <div className="flex space-x-1">
                                  <Button size="sm" onClick={() => saveEdit(envio.id)}>Salvar</Button>
                              ...
                              <div>
                                ...
                                {envio.observacao_admin && <p className="text-xs text-muted-foreground mt-1 max-w-[200px] whitespace-normal">{envio.observacao_admin}</p>}
                              </div>
                            )}
                          </TableCell>
                          <TableCell className="text-sm">{new Date(envio.created_at).toLocaleDateString('pt-BR')}</TableCell>
                          <TableCell>
                            <div className="flex space-x-1">
                              <Button size="icon" variant="outline" onClick={() => downloadFile(envio)}><Download className="h-4 w-4" /></Button>
                              <Button size="icon" variant="outline" onClick={() => startEdit(envio)} disabled={editingEnvio === envio.id}><Edit2 className="h-4 w-4" /></Button>
                              <Button size="icon" variant="outline" onClick={() => setChatChapterId(envio.id)}><MessageCircle className="h-4 w-4" /></Button>
                            </div>
                          </TableCell>
                        </TableRow>
                      ))}
                  ...
                  {filteredEnvios.length === 0 && !isLoading && (
                    <div className="text-center py-8">
                      <FileText className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                      <p className="text-muted-foreground">{searchTerm || statusFilter !== 'all' || responsibleFilter !== 'all' ? 'Nenhum envio encontrado com os filtros aplicados.' : 'Nenhum envio recebido ainda.'}</p>
                    </div>
                  )}
                </div>
              )}
